      ...
      <div class="text-white/70 flex flex-col items-center">
        <div class="p-5 flex flex-col items-center w-[360px] md:w-[380px] border-2 border-white/30 rounded-lg">
          ...
          <label class="form-control w-full max-w-xs">
            ...
            <input input v-model="formData.name" type="text" placeholder="Name"
              class="input bg-transparent input-sm input-ghost input-bordered rounded-sm w-full max-w-xs" />
            <span class="text-right text-sm text-error" v-if="errorMessage.name">{{ errorMessage.name }}</span>
          </label>
          ...
          <label class="form-control w-full max-w-xs">
            ...
            <input input v-model="formData.confirm_password" type="password" placeholder="Confirm Password"
              class="input bg-transparent input-sm input-ghost input-bordered rounded-sm w-full max-w-xs" />
            <span class="text-right text-sm text-error" v-if="errorMessage.confirm_password">{{ errorMessage.confirm_password }}</span>
          </label>

          <button @click="handleLogin" class=" btn btn-ghost btn-outline w-[319px] mt-5 rounded-sm">Create User
          </button>
...
definePageMeta({
  ...
  middleware: [async () => {
    ...
    const isExist = await AuthStore.getUser()
